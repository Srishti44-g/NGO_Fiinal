...
function changeLanguage(langCode) {
    ...
    if (langCode === 'en') {
        // For English, just clear translations and reload
        location.reload();
        return;
    }
    ...
    try {
        ...
        if (googleFrame) {
            googleFrame.contentWindow.document.getElementsByClassName('goog-te-combo')[0].value = langCode;
            googleFrame.contentWindow.document.getElementsByClassName('goog-te-combo')[0].dispatchEvent(new Event('change'));
        } else {
            // If frame not found, reinitialize translator
        ...
        console.warn('Translation refresh failed, reloading page');
        location.reload();
    }
